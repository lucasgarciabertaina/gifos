...
function dataTokenToArray(dataToken) {
  let array = [];
  let word = "";
  if (dataToken != 0 && dataToken != null) {
    for (let i = 0; i < dataToken.length + 1; i++) {
      if (dataToken[i] == "," || i == dataToken.length) {
        array.push(word);
        word = "";
      } else {
        word += dataToken[i];
    ...
    console.log(`function dataTokenToArray: ERROR, dataToken is ${dataToken}`);
  }
  return array;
}
...
function gifoDeleteRepited(arrayToken) {
  let tokenContent = [];
  for (let i = 0; i < arrayToken.length; i++) {
    comparator = arrayToken[i];
    let same = true;
    for (let j = 0; j < arrayToken.length; j++) {
      if (i != j) {
        if (comparator == arrayToken[j]) {
          same = false;
        }
      }
    }
    if (!same) {
      tokenContent.push(comparator);
    }
  }
  return tokenContent;
}

function tokenDataToContent(tokenData) {
  if (tokenData != null) {
    let array = dataTokenToArray(tokenData);
    let desision = array.length > 1 ? gifoDeleteRepited(array) : array;
    console.log(desision);
    return desision;
  } else {
    console.log(
      `function tokenDataToContent: ERROR ${tokenData} array is NULL`
    );
  }
}
...
if (localStorage.getItem("tokens") != null) {
  tokenData = localStorage.getItem("tokens");
  tokenContent = tokenDataToContent(tokenData);
}
...
/* function tokenDataToContent(tokenData) {
  if (tokenData != null) {
    let array = dataTokenToArray(tokenData);
    console.log(array);
    let tokenContent = gifoDeleteRepited(array);
  ...
  if (gifos.length == 0) {
    ...
    for (i; i < j; i++) {
      ...
      gifoImage.addEventListener("click", () => {
        ...
        textContainer.setAttribute(
          "class",
          "elments-gifo-Expand__text-container"
        );
...
let i = 0;
let j = 3;
callApi(endPointTrending).then((json) => {
